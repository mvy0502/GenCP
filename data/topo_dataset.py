...
class TopoDataset(BaseDataset):
    """
    Pix2Pix TOPO Dataset where input A = RGB + TOPO (4 canaux)
    and output B = RGB (3 canaux)
    """
    ...
    def __init__(self, opt):
        ...
        assert len(self.A_rgb_paths) == len(self.A_topo_paths) == len(self.B_paths), \
            f"Folders should have the same length" \
            f"A: {len(self.A_rgb_paths)}, TOPO: {len(self.A_topo_paths)}, B: {len(self.B_paths)}"
        ...
        for i in range(len(self.A_rgb_paths)):
            ...
            b_name = os.path.basename(self.B_paths[i])
            
            a_base = os.path.splitext(a_name)[0]
            topo_base = os.path.splitext(topo_name)[0]
            b_base = os.path.splitext(b_name)[0]
            
            if a_base != topo_base or a_base != b_base:
                raise ValueError(
        ...
        self.transform_B = get_transform(opt, grayscale=False)
    ...
    def __getitem__(self, index):
        ...
        B_tensor = self.transform_B(B)              # [3, H, W]
